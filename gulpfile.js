...
gulp.task('scss', ['scss_min'], function () {
    return gulp.src(cnf.path.app + cnf.path.css.in)
                ...
                .pipe(if_(cnf.prop.css.sourcemap, sourcemaps.init()))
                    ...
                    .pipe(if_(cnf.prop.css.concat, concat('styles.css').on('error', console.log)))
    ...
    if(cnf.prop.css.min){
        return gulp.src(cnf.path.app + cnf.path.css.in)
                    ...
                    .pipe(if_(cnf.prop.css.sourcemap, sourcemaps.init()))
                        ...
                        .pipe(prefix("last 1 version", "> 1%", "ie 8", "ie 7"))
                        .pipe(if_(cnf.prop.css.concat, concat('styles.min.css').on('error', console.log)))
                    .pipe(if_(cnf.prop.css.sourcemap, sourcemaps.write(cnf.path.css.sourcemap)))
                    .pipe(gulp.dest(cnf.path.dst + cnf.path.css.out))
...
gulp.task('mdi', function() {
    if(cnf.prop.fonts.mdi){
        console.log('>>>> mdi start');
        gulp.src(cnf.path.fonts.mdi)
            .pipe(gulp.dest(cnf.path.dst + cnf.path.fonts.out + 'mdi/'));
    }
});